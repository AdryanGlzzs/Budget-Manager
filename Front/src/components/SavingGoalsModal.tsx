import { ShieldCheck, Car, Plane, PiggyBank, X } from "lucide-react";
...
export const SavingsGoalsModal = ({IsOpen, OnClose}: SavingsGoalsModalProps) => {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#0a0a18] border border-white/10 rounded-2xl p-7 w-full max-w-[480px] mx-4">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <p className="text-[11px] text-white/40 uppercase tracking-widest mb-1">Nova meta</p>
            <h2 className="text-[18px] font-semibold text-white">Criar objetivo financeiro</h2>
          </div>
          <button 
          onClick={() => OnClose()}
        ...
        <div className="mb-5">
          <label className="text-[12px] text-white/45 block mb-2">Nome da meta</label>
          <input
            type="text"
        ...
        <div className="grid grid-cols-2 gap-4 mb-5">
          <div>
            <label className="text-[12px] text-white/45 block mb-2">Valor alvo (R$)</label>
            <input
              type="text"
              placeholder="0,00"
              className="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 text-[14px] text-white/85 placeholder:text-white/20 outline-none focus:border-white/20 transition-colors"
          ...
          <div>
            <label className="text-[12px] text-white/45 block mb-2">Valor inicial (R$)</label>
            <input
              type="text"
              placeholder="0,00"
              className="w-full bg-white/[0.04] border border-white/10 rounded-xl px-4 py-3 text-[14px] text-white/85 placeholder:text-white/20 outline-none focus:border-white/20 transition-colors"
        ...
        <div className="mb-5">
          <label className="text-[12px] text-white/45 block mb-3">Ícone da meta</label>
          <div className="flex gap-3">
            {[
              ...
              >
                <Icon className={`w-[18px] h-[18px] ${active ? "text-purple-400" : "text-white/40"}`} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-7">
          <label className="text-[12px] text-white/45 block mb-3">Cor da meta</label>
          <div className="flex gap-3">
            {[
